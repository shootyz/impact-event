...
function SuccessPageInner() {
  ...
  useEffect(() => {
    ...
    setIsAndroidPlatform(/Android/.test(navigator.userAgent));
  }, []);
      ...
      <div className="w-full max-w-md">
        ...
        {isApplePlatform && (
          <a
            ...
            className="mt-3 w-full py-3.5 rounded-xl font-semibold text-sm tracking-widest uppercase flex items-center justify-center gap-2 transition"
            style={{ background: "#000000", color: "white", textDecoration: "none" }}
        ...
        {isAndroidPlatform && (
          <a
            href={`/api/google-wallet/${token}`}
            className="mt-3 w-full py-3.5 rounded-xl font-semibold text-sm tracking-widest uppercase flex items-center justify-center gap-2 transition"
            style={{ background: "white", color: "var(--ig-navy)", border: "1.5px solid var(--ig-navy)", textDecoration: "none" }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(30,50,99,0.06)"; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "white"; }}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M6 6h12a2 2 0 012 2v8a2 2 0 01-2 2H6a2 2 0 01-2-2V8a2 2 0 012-2z" />
            </svg>
            {lang === "de" ? "Zu Google Wallet hinzufügen" : lang === "fr" ? "Ajouter à Google Wallet" : "Add to Google Wallet"}
          </a>
        )}
        ...
        {!showWhatsApp ? (
          ...
          >
            ...
            {lang === "de" ? "Ticket per WhatsApp senden" : lang === "fr" ? "Envoyer le billet par WhatsApp" : "Send ticket via WhatsApp"}
          </button>
        ) : (
